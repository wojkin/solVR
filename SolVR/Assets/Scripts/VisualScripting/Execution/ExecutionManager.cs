    ...
    public class ExecutionManager : MonoBehaviour
    {
        ...
        private class BlockExecutionThread
        {
            ...
            public BlockExecutionThread(ExecutionManager manager, Block currentBlock)
            {
                ...
                _threadId = manager._robot.CreateThread(RobotThreadStateChangeHandler);
                _manager = manager;
            ...
            private IEnumerator ExecuteBlocks()
            {
                while (_currentBlock != null && !_finishExecution)
                {
                    ...
                    if (_manager._executionState == ExecutionState.Paused)
                        yield break;
            ...
            private IEnumerator ExecuteBlock()
            {
                ...
                {
                    case EndBlock _:
                        // in case of an end block stop execution and set the execute next block flag to true so the
            ...
            {
                if (_manager._pauseOnNextStep)
                {
                    _manager.PauseExecution();
                    _manager._pauseOnNextStep = false;
                }
            }
            ...
            private void RobotThreadStateChangeHandler(ThreadState changedTo)
            {
                switch (changedTo)
                {
