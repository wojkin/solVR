using System;

namespace VisualCoding.ConditionExpression.Conditions
{
    /// <summary>
    /// Class representing condition as a result of a logic operation on two conditions.
    /// </summary>
    public class TwoConditionLogicOperation : Condition
    {
        public Condition left; // condition that is on the left of the logic operand
        
        public Condition right; // condition that is on the right of the logic operand

        public LogicOperation Operation { get; set; } // logic operation performed on right and left condition
        
        /// <summary>
        /// Check and returns boolean that is a result of logic operation.
        /// </summary>
        /// <returns>A boolean result of logic operation on left and right result of checked condition.</returns>
        /// <exception cref="ArgumentOutOfRangeException">Throws exception when <c>Operation</c> is not handled.</exception>
        public override bool Check()
        {
            return Operation switch
            {
                LogicOperation.And => left.Check() &&  right.Check(),
                LogicOperation.Or => left.Check() ||  right.Check(),
                _ => throw new ArgumentOutOfRangeException()
            };
        }
    }

    /// <summary>
    /// Enum that represents a logic operation type. 
    /// </summary>
    public enum LogicOperation
    {
        And, Or
    }
}